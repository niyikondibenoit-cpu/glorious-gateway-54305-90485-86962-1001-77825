-- Update verify_flexible_login function to handle empty password_hash as default "123"
...
BEGIN
  ...
  INTO v_user_record
  FROM admins 
  WHERE email = p_identifier AND (password_hash = p_password OR (password_hash IS NULL AND p_password = '123'));
  ...
  INTO v_user_record
  FROM teachers 
  WHERE email = p_identifier AND (password_hash::text = p_password OR (password_hash IS NULL AND p_password = '123'));
