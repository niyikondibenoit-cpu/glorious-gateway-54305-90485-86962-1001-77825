-- Add debug info to verify_flexible_login function
...
BEGIN
  ...
  IF v_admin IS NOT NULL THEN
    ...
    IF v_admin.password_hash = p_password THEN
      ...
      RETURN v_result;
    ELSE
      v_result := json_build_object('success', false, 'message', 'Invalid admin credentials', 'debug', 'password mismatch');
      RETURN v_result;
    END IF;
  END IF;
  ...
  IF v_teacher IS NOT NULL THEN
    ...
    IF v_teacher.password_hash IS NULL OR v_teacher.password_hash = '' THEN
      ...
      IF p_password = '123' THEN
        ...
        RETURN v_result;
      ELSE
        v_result := json_build_object('success', false, 'message', 'Invalid teacher credentials (default)', 'debug', 'password mismatch for default');
        RETURN v_result;
      END IF;
    ELSE
      ...
      IF v_teacher.password_hash = p_password THEN
        ...
        RETURN v_result;
      ELSE
        v_result := json_build_object('success', false, 'message', 'Invalid teacher credentials (custom)', 'debug', 'password mismatch for custom');
        RETURN v_result;
      END IF;
    END IF;
  END IF;
  ...
  IF v_student IS NOT NULL THEN
    ...
    IF v_student.password_hash IS NULL OR v_student.password_hash = '' THEN
      ...
      IF p_password = '123' THEN
        ...
        RETURN v_result;
      ELSE
        v_result := json_build_object('success', false, 'message', 'Invalid student credentials (default)', 'debug', 'password mismatch');
        RETURN v_result;
      END IF;
    ELSE
      -- Custom password is set, check against stored password
      ...
      ELSIF LENGTH(v_student.password_hash) > 10 AND v_student.password_hash = extensions.crypt(p_password, v_student.password_hash) THEN
        ...
        RETURN v_result;
      ELSE
        v_result := json_build_object('success', false, 'message', 'Invalid student credentials (custom)', 'debug', 'password mismatch');
        RETURN v_result;
      END IF;
    END IF;
  END IF;
