...
-- Update electoral votes with new voter emails from students
UPDATE public.electoral_votes
SET voter_email = s.email
FROM public.students s
WHERE s.id = electoral_votes.voter_id;

-- Update electoral votes with new voter emails from teachers
UPDATE public.electoral_votes
SET voter_email = t.email
FROM public.teachers t
WHERE t.id = electoral_votes.voter_id;

-- Update electoral votes with new voter emails from admins
UPDATE public.electoral_votes
SET voter_email = a.email
FROM public.admins a
WHERE a.id = electoral_votes.voter_id;
